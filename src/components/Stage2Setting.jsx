...
function Stage2Setting() {
    ...
    const handleUpdateClick = () => {
        if (password === '123456') {
            const postData = {
                TotalDay_2: parseInt(totalDay, 10),
                TimeBegin_2: parseInt(timeBegin, 10),
                TimeStop_2: parseInt(timeStop, 10),
                Times_Watering_2: parseInt(timesWatering, 10),
                TotalML_2: parseFloat(totalML)
            };

            fetch('https://ap-southeast-1.aws.data.mongodb-api.com/app/application-0-cavmt/endpoint/VinhPostReactFlow_2', {
                method: 'POST',
                headers: {
    ...
    return (
        <div style={{ border: '1px solid black', padding: '10px', width: '30%' }}>
            <h3>Cài đặt lưu lượng nước giai đoạn 2</h3>
